...
module.exports.createBuyOrder = async (req,res) =>{
    try {
       ...
       const products = await fetch('SELECT * FROM portfolio WHERE id = ?', [req.body?.productid])
       ...
       await save('UPDATE `accounts` SET `balance` = ? WHERE `userid` = ?',[charge,req.user.id])
       ...
       await save('UPDATE portfolio SET qty = ? WHERE id = ?',[qty, req.body.productid])
       //    'userid', 'productid', 'quantity', 'price', 'expiry', 'type', 'status'
       res.send({status:'ok'})
    } catch (error) {
...
module.exports.createSellOrder = async (req,res) =>{
    try {
       ...
       if(req.body?.receipt_id != 0 && !isNaN(req.body?.receipt_id)){
         const receipts = await fetch('SELECT * FROM `receipts` WHERE id = ?', [req.body?.receipt_id])
         ...
         const orders = await fetch('SELECT * FROM `order` WHERE id = ?', [receipt.orderid])
...
module.exports.acceptSellOrder = async (req,res) =>{
    try {
       const orders = await fetch('SELECT * FROM `order` WHERE id = ?', [req.params.id])
      ...
      //  'userid', 'productid', 'quantity', 'price', 'expiry', 'type', 'status'
      const portfolio = await fetch('SELECT * FROM portfolio WHERE id = ?', [order.productid])
      const user = await fetch('SELECT * FROM users WHERE id = ?',[order.userid])
...
module.exports.acceptSellOrderListing = async (req,res) =>{
    try {
       ...
       const order = orders[0]
       const portfolio = await fetch('SELECT product, qty FROM portfolio WHERE id = ?', [order.productid])
...
module.exports.rejectSellOrder = async (req,res) =>{
    try {
       ...
       const order = orders[0]
       const portfolio = await fetch('SELECT qty FROM portfolio WHERE id = ?', [order.productid])
...
module.exports.rejectBuyOrder = async (req,res) =>{
    try {
       ...
       const balance = await fetch('SELECT * FROM `accounts` WHERE `userid` = ?', [order.userid])
       const bal = balance[0].balance
       ...
       await save('UPDATE `accounts` SET `balance` = ? WHERE `userid` = ?',[charge,order.userid])

       await save('UPDATE `order` SET ? WHERE id = ?',[{status:2}, order.id])
       const products = await fetch('SELECT * FROM portfolio WHERE id = ?', [order.productid])
      ...
      const user = await save('SELECT * FROM users WHERE id = ?',[order.userid])
...
module.exports.getOrders = async (req,res) =>{
    try {
       sql = 'SELECT `order`.`userid`,`order`.`productid`,`receipts`.`qty` AS `quantity` ,`receipts`.`price`,`order`.`expiry`,`order`.`type`,`order`.`status`,`order`.`createdAt`,`receipts`.`product_name` ,`receipts`.`tx` FROM `order` LEFT JOIN `receipts` ON `order`.`id` = `receipts`.`orderid` WHERE `order`.`userid`= ? ORDER BY `receipts`.`createdAt` DESC'
...
module.exports.getOrdersByStatusAdmin = async (req,res) =>{
    try {
       sql = 'SELECT `order`.`id`, `order`.`userid`,`order`.`productid`,`order`.`quantity`,`order`.`price`,`order`.`expiry`,`order`.`type`,`order`.`status`,`order`.`createdAt`,`portfolio`.`product` FROM `order` LEFT JOIN `portfolio` ON `order`.`productid` = `portfolio`.`id` WHERE `order`.`status` = ?  ORDER BY `order`.`createdAt` DESC'
...
module.exports.getReceipt = async (req,res) =>{
    try {
       sql = 'SELECT * FROM `receipts` WHERE `tx` = ?'
...
module.exports.getReceipts = async (req,res) =>{
    try {
       sql = 'SELECT `receipts`.`id`, `receipts`.`product_name`,`receipts`.`price`, `receipts`.`qty` FROM `order` LEFT JOIN `receipts` ON `order`.`id` = `receipts`.`orderid` WHERE `receipts`.`status` = 1 AND `order`.`userid` = ? AND `order`.`productid` = ?'
