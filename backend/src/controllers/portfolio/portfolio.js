...
module.exports.deletePortfolio = async (req,res) =>{
    try {
       await save('DELETE FROM "portfolio" WHERE id = $1', [req.params.id])
       res.send({success: true})
    } catch (error) {
       handleError(error, res)
    }
...
module.exports.getPortfolio = async (req,res) =>{
    try {
      const orders = await fetch(
        'SELECT p.id, p.product, p.cost, p.sellingprice, COALESCE(SUM(COALESCE(r.qty, 0)), 0) AS qty, o.userid FROM "portfolio" p LEFT JOIN "order" o ON p.id = o.productid AND o.userid = $1 LEFT JOIN "receipts" r ON o.id = r.orderid AND r.status = 1 GROUP BY p.id, o.userid, p.product, p.cost, p.sellingprice',
        [req.user.id]
      )
      res.send(orders)
    } catch (error) {
...
module.exports.getProduct = async (req,res) =>{
    try {
       const prt = await fetch('SELECT * FROM "portfolio" WHERE "product" = $1', [req.params.product])
